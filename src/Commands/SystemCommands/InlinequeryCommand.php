...
class InlinequeryCommand extends SystemCommand
{
	...
	if($query == "") {
	        $array_article[] = new InlineQueryResultArticle(['id' => "$id", 'title' => "Info message", 'message_text' => "Info about this bot.", 'input_message_content' => [ 'message_text' => 'This bot will create a big ASCII version of the text you send him. To use simply type
...
Source code @ http://github.com/danog/figletsbot', 'parse_mode' => 'Markdown' ] ]);
	} else {
		$url = shell_exec('export LANG=it_IT.UTF-8;export LC_ALL=it_IT.UTF-8;txt=$(echo '.escapeshellarg($query).' | figlet); echo "$txt" 2>&1 | aha -b 2>&1 | sed \'s/^/<style>span { font-size: 400%; }<\/style>/g\' | wkhtmltoimage --width $(echo "$txt" | wc -L) --quality 100 -q -f jpg - - | curl -T - chunk.io | tr -d "\n"') . '.jpg';
		$data_array = array("id" => "$id",
'title' => "figletified text",
'description' => $query,
"photo_url" => $url,
"thumb_url" => $url );
